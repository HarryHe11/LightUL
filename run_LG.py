...
def run_model(train_records, val_records, test_records, unlearn_records, sys_paras, params):
    ...
    if sys_paras.unlearn == 'none' and sys_paras.train:
        ...
        logging.info(f'INFO: A fully-trained model is needed before conducting unlearning! START TRAINING!')
        ...
        logging.info(f'TRAINING FINISHED! CURRENT TIME: {(time.time() - start_time) / 60:.2f}min')
    if os.path.exists(f'checkpoints/{sys_paras.dataset}/none/ori/{sys_paras.base}-0.pt'):
        logging.info("Load a pre-trained model")
    ...
    logging.info(f'UL AUC before unlearning: {before_ul_auc:.4f}')
    logging.info(f'ACC AUC before unlearning: {before_acc_auc:.4f}')
    logging.info(f'NDCG@20 before unlearning: {before_ndcg:.4f}')
    if sys_paras.unlearn == 'retrain':
        ...
        model = LightGCN(params, sys_paras).to(params['device'])
        if sys_paras.train:
            logging.info(f"[RETRAIN] START TRAINING. CURRENT TIME: {(time.time() - start_time) / 60:.2f}min.")
            train_lg_model(model, graph, use_train_records, val_records, test_records, sys_paras, params)
            logging.info(f"[RETRAIN] TRAINING FINISHED. TOTAL TIME: {(time.time() - start_time) / 60:.2f}min.")
        ...
        if sys_paras.train:
            logging.info(f"[RecEraser] START TRAINING. CURRENT TIME: {(time.time() - start_time) / 60:.2f}min.")
            ...
            logging.info(f"[RecEraser] TRAINING FINISHED. TOTAL TIME: {(time.time() - start_time) / 60:.2f}min.")
        ...
        if sys_paras.train:
            logging.info(f"[LASER] START TRAINING. CURRENT TIME: {(time.time() - start_time) / 60:.2f}min.")
            conduct_laser(model, graph, use_train_records, unlearn_records, val_records, test_records, tr_rds,
                          sys_paras, params, start_time)
            logging.info(f"[LASER] TRAINING FINISHED. TOTAL TIME: {(time.time() - start_time) / 60:.2f}min.")
        ...
        if sys_paras.train:
            logging.info(f'AUC before unlearning: {test_model_auc(model, val_records, graph):.4f}')
            logging.info(f'NDCG@20 before unlearning: {test_model_ndcg(model, use_train_records, val_records, graph):.4f}')
            logging.info(f"[RECUL] START TRAINING. CURRENT TIME: {(time.time() - start_time) / 60:.2f}min.")
            conduct_recul_unlearn(model, graph, use_train_records, val_records, test_records, unlearn_records,
                                  sys_paras,
                                  params)
            logging.info(f"[RECUL] TRAINING FINISHED. TOTAL TIME: {(time.time() - start_time) / 60:.2f}min.")
        ...
        if sys_paras.train:
            logging.info(f"[IFRU] START TRAINING. CURRENT TIME: {(time.time() - start_time) / 60:.2f}min.")
            conduct_ifru_unlearn(model, graph, use_train_records, val_records, test_records, unlearn_records,
                                 sys_paras,
                                 params)
            logging.info(f"[IFRU] TRAINING FINISHED. TOTAL TIME: {(time.time() - start_time) / 60:.2f}min.")
    ...
    if os.path.exists(
        ...
        logging.info(f'ERROR: No unlearned model!')
    ...
    logging.info(f'After unlearning: ACC AUC: {acc_auc:.4f}, UL AUC: {ul_auc:.4f}, ACC NDCG@20: {acc_ndcg:.4f}')
    ...
    if sys_paras.test_sim:
        ...
        if os.path.exists(
            ...
            logging.info("Load a re-train model")
